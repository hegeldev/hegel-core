...
Performance improvements: a buffered packet reader (TrioBufferedReader) reduces epoll/reschedule overhead by reading in 65 kB chunks instead of per-field; test-case I/O now crosses the trio→thread boundary once per test case instead of once per packet. The dead unix-socket CLI path has been removed. The `--stdio` flag is still accepted for backwards compatibility but is now a no-op (stdio is the default).
