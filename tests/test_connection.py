...
def test_read_request_sync_timeout(socket_pair):
    """read_request_sync raises TimeoutError when the queue stays empty."""
    server_socket, client_socket = socket_pair
    errors = []
    ...
    async def server_side(conn):
        await conn.receive_handshake()
        stream = await conn.new_stream()
        stream._sync_requests = queue.SimpleQueue()

        def check():
            try:
                stream.read_request_sync(timeout=0.1)
            except TimeoutError as e:
                errors.append(e)

        await trio.to_thread.run_sync(check)
...
def test_read_request_sync_connection_error(socket_pair):
    """read_request_sync re-raises exceptions deposited by the reader task."""
    server_socket, client_socket = socket_pair
    errors = []
    ...
    async def server_side(conn):
        ...
        stream = await conn.new_stream()
        stream._sync_requests = queue.SimpleQueue()
        stream._sync_requests.put(ConnectionError("injected error"))

        def check():
            try:
                stream.read_request_sync()
            except ConnectionError as e:
                errors.append(e)
    ...
    assert len(errors) == 1
    assert "injected error" in str(errors[0])


def test_stream_close_signals_sync_requests(socket_pair):
    """stream.close() wakes a thread blocked in read_request_sync."""
    server_socket, client_socket = socket_pair
    errors = []
    ...
    async def server_side(conn):
        ...
        def wait_for_packet():
            try:
                stream.read_request_sync(timeout=5.0)
            except ConnectionError as e:
                errors.append(e)

        async with trio.open_nursery() as nursery:
            nursery.start_soon(trio.to_thread.run_sync, wait_for_packet)
    ...
    assert len(errors) == 1
    assert "Connection closed" in str(errors[0])
